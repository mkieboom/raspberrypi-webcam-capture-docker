...
while true; do
  ...
  #then
    ...
    then
      ...
      curl -i -X PUT "http://$MAPR_HOST:14000/webhdfs/v1/webcam/$filename?op=CREATE&permission=444&user.name=mapr"
      echo 'curl -i -X PUT "http://$MAPR_HOST:14000/webhdfs/v1/webcam/$filename?op=CREATE&permission=444&user.name=mapr"'
...
#      curl -i -X PUT "http://$MAPR_HOST:14000/webhdfs/v1/webcam/$filename?op=SETPERMISSION&permission=444&user.name=mapr"
#      echo 'curl -i -X PUT "http://$MAPR_HOST:14000/webhdfs/v1/webcam/$filename?op=SETPERMISSION&permission=444&user.name=mapr"'

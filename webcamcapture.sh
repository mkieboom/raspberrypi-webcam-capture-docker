...
while true; do
  ...
  curl -i -X PUT "http://192.168.1.221:14000/webhdfs/v1/webcam/$filename?op=CREATE&user.name=mapr"
  curl -i -X PUT -T $outputfolder/$filename -H "Content-Type:application/octet-stream" "http://192.168.1.221:14000/webhdfs/v1/webcam/$filename?op=CREATE&overwrite=true&permission=550&data=true&user.name=mapr"
  ...
  curl -X POST -H "Content-Type: application/vnd.kafka.json.v1+json" \
     ...
     http://$MAPR_USER:$MAPR_PASSWORD@192.168.1.221:8082/topics/%2F$MAPR_STREAM%3A$MAPR_STREAM_TOPIC
